# -*- coding: utf-8 -*-
"""
Created on Sat Jun 01 14:14:07 2019
...
data1 = numpy.loadtxt("input_output_files/rho_"+str(rho)+"/gamma_"+str(gamma)+"/contour/nu1_contour_c1="+str(c1_short)+"_c0="+str(c0_short)+"_18000points_iter"+str(iteration)+".txt",float)
data2 = numpy.loadtxt("input_output_files/rho_"+str(rho)+"/gamma_"+str(gamma)+"/contour/nu2_contour_c1="+str(c1_short)+"_c0="+str(c0_short)+"_18000points_iter"+str(iteration)+".txt",float)
data3 = numpy.loadtxt("input_output_files/rho_"+str(rho)+"/gamma_"+str(gamma)+"/mapping/mapping_output_nu1_c1="+str(c1_short)+"_c0="+str(c0_short)+"_18000points_iter"+str(iteration)+".txt",float)
data4 = numpy.loadtxt("input_output_files/rho_"+str(rho)+"/gamma_"+str(gamma)+"/mapping/mapping_output_nu2_c1="+str(c1_short)+"_c0="+str(c0_short)+"_18000points_iter"+str(iteration)+".txt",float)
contr = numpy.loadtxt("input_output_files/rho_"+str(rho)+"/gamma_"+str(gamma)+"/contour/nu1_contour_c1="+str(c1_short)+"_c0="+str(c0_short)+"_18000points_iter"+str(iteration)+".txt",float)
f_x_input = numpy.loadtxt("input_output_files/rho_"+str(rho)+"/gamma_"+str(gamma)+"/density/f_y_calculation_newton_rapson_method_2_gamma="+str(gamma)+"_18000points_correction4_iter"+str(iteration-1)+".txt",float)
f_out=file("input_output_files/rho_"+str(rho)+"/gamma_"+str(gamma)+"/density/f_y_calculation_newton_rapson_method_2_gamma="+str(gamma)+"_18000points_correction4_iter"+str(iteration)+".txt","w")
...
#f_x = ((deriv_V*x)/gamma)-((1.0-gamma)/gamma)     # Step(1): Initialization
f_x_2 = f_x_input[:, 1]    # f(x) from previous iteration
...
for l in range(len(x)):    # function len() on array gives no. of rows of array
    first_term = f_x_2[l]    # first term in formula for F(X) in notebook
    second_term = -((deriv_V[l])/gamma)    # second term in formula for F(X) in notebook
#    second_term = -((f_x[l])/gamma)+((1.0-gamma)/gamma)    # second term in formula for F(X) in notebook
#    funct1 = ((1.0-gamma)/(2*(numpy.pi)*gamma))*f_x*((((1.0/(h_x_plus[l]-hbar_x))+(1.0/(hbar_x_plus[l]-hbar_x)))*deriv_hbar).imag)
    funct1 = ((1.0-gamma)/(2*(numpy.pi)*gamma))*f_x_2*((((1.0/(h_x_plus[l]-hbar_x))+(1.0/(hbar_x_plus[l]-hbar_x)))*deriv_hbar).imag)
...
f_x_next = f_x_2 - delta_X
max_error = numpy.amax(abs(f_x_next - f_x_2))
...
for m in range(len(x)):    # function len() on array gives no. of rows of array
    f_out.write(str(x[m])+" "+str(f_x_2[m])+'\n')
